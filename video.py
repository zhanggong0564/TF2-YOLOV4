...
if __name__ == '__main__':
    ...
    if cfg.yolotiny:
        yolov4_weight = 'yolov4_tiny_car.h5'
        ...
        while True:
            ...
            if scores.numpy().all():
                for bbox,score,cls in zip(boxes,scores,classes):
                    ...
                    x1,y1,x2,y2 = bbox.numpy()*[frame.shape[1],frame.shape[0],frame.shape[1],frame.shape[0]]
                    cv2.rectangle(frame,(int(x1),int(y1)),(int(x2),int(y2)),(0,255,0),1)
                    cv2.putText(frame,classes_dict[int(cls.numpy())]+':'+str(score.numpy()),(int(x1),int(y1)),cv2.FONT_HERSHEY_SCRIPT_COMPLEX,0.9,(0,225,0))
